...
(function( $ ) {
    $(function() {
        var slug_cont = $("[data-ng-switch-when=text]");
        $(slug_cont).each(function(){
            if ( $(this).children('input').length > 0 ) {
                var title_input = $(this).children('input');
                if ( $(this).find('input[app-slug]').length > 0 ) {
                    var input_slug = $(this).find('input[app-slug]');
                    $(title_input).off();
                    $(title_input).keyup(function(){
                        $(input_slug).val( url_slug($(this).val()) );
                    });
                }
            }
        });
